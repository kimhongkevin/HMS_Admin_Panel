            ...
            <div class="p-6 sm:p-8 bg-white border-b border-gray-200">
                ...
                <form method="POST" action="{{ route('admin.departments.update', $department) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <input id="name" name="name" type="text" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" value="{{ old('name', $department->name) }}" required autofocus />
                        @error('name') <p class="text-sm text-red-600 mt-2">{{ $message }}</p> @enderror
                    </div>

                    <div class="mb-4">
                        <label for="code" class="block font-medium text-sm text-gray-700">Code <span class="text-red-500">*</span></label>
                        <input id="code" name="code" type="text" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm uppercase" value="{{ old('code', $department->code) }}" required maxlength="50" />
                        <p class="text-xs text-gray-500 mt-1">Short, unique code (e.g., CARDIO, NEURO). Will be saved in uppercase.</p>
                        @error('code') <p class="text-sm text-red-600 mt-2">{{ $message }}</p> @enderror
                    </div>

                    <div class="mb-4">
                        <label for="description" class="block font-medium text-sm text-gray-700">Description</label>
                        <textarea id="description" name="description" rows="3" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">{{ old('description', $department->description) }}</textarea>
                    ...
                    <div class="mb-6 flex items-center">
                        <input id="is_active" name="is_active" type="checkbox" class="h-4 w-4 text-purple-600 border-gray-300 rounded focus:ring-purple-500" value="1" {{ old('is_active', $department->is_active) ? 'checked' : '' }}>
